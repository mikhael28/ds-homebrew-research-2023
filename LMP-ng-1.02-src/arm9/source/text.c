	...
	if(mask == 0)
		...
		for(i = 0; i < h; i++)
			for(j = 0; j < w; j++) {
				...
				if(color != 0) {
					if(color > 24)
						buffer[(y_offset + i) * surface_width + x_offset + j] = RGB15(31,31,31) | BIT(15);
					else if(color > 16)
						buffer[(y_offset + i) * surface_width + x_offset + j] = RGB15(28,28,28) | BIT(15);
					else if(color > 8)
						buffer[(y_offset + i) * surface_width + x_offset + j] = RGB15(25,25,25) | BIT(15);
