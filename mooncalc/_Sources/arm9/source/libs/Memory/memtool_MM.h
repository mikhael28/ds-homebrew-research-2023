...
TMM MM_Temp,MM_System,MM_SystemAfter,MM_Skin,MM_DLLImage,MM_DLLSound,MM_DLLDPG,MM_SKKOSK,MM_Process;
...
void MM_Init(void)
{
  ...
  safemalloc_CallBack_RequestFreeMemory_NDSFiles=NULL;
  ...
  for(u32 mmidx=0;mmidx<MMEnumCount;mmidx++){
    ...
    switch(mmidx){
      ...
      case 7: pMM=&MM_SKKOSK; pMM->pName="PlayList"; break;
      case 8: pMM=&MM_Process; pMM->pName="Process"; break;
    }
    if(pMM==NULL) StopFatalError(19401,"Unknown memory manager ID.\n");
    MMEnum[mmidx]=pMM;
    pMM->ListCount=0;
    pMM->pLists=NULL;
  }
...
static void MM_Set(TMM *pMM,const char *filename,int linenum,const char *funcname,u32 adr,u32 size)
{
  ...
  { // 拡張するための空きメモリを先にチェック
    u32 reqsize=(pMM->ListCount+MMListExpandUnitCount)*sizeof(TMM_List);
