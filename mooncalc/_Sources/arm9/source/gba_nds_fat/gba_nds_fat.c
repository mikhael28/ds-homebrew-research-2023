...
static u32 filesysTotalSize;

// Info about FAT
static u32 fatLastCluster;
static u32 fatFirstFree;

// fatBuffer used to reduce wear on the CF card from multiple writes
static char fatBuffer[BYTE_PER_READ];
static u32 fatBufferCurSector;
...
bool FAT_WriteFatEntry (u32 cluster, u32 value)
{
  ...
  if ((cluster < 0x0002) || (cluster > fatLastCluster)) return false;
...
u32 FAT_ReadWriteFatEntryBuffered (u32 cluster, u32 value)
{
  ...
  if ((cluster < 0x0002) || (cluster > fatLastCluster)) return CLUSTER_FREE;
  ...
  switch (filesysType) {
    ...
    case FS_FAT12:
      ...
      if (oldValue >= 0x0FF7) oldValue = CLUSTER_EOF;
    ...
    case FS_FAT16:
      ...
      if (oldValue >= 0xFFF7) oldValue = CLUSTER_EOF;
    ...
    case FS_FAT32:
      ...
      if (oldValue >= 0x0FFFFFF7) oldValue = CLUSTER_EOF;
...
{
  if (cluster > fatLastCluster) return CLUSTER_FREE;

  { // Check if the cluster already has a link, and return it if so
    u32 curLink = FAT_NextCluster (cluster);
    if ((curLink >= CLUSTER_FIRST) && (curLink <= fatLastCluster)) return curLink;  // Return the current link - don't allocate a new one
  }
  
  // Get a free cluster
  u32 firstFree = FAT_FirstFreeCluster();

  // If couldn't get a free cluster then return
  if (firstFree == CLUSTER_EOF) return CLUSTER_FREE;

  if ((cluster >= CLUSTER_FIRST) && (cluster <= fatLastCluster)) FAT_WriteFatEntry (cluster, firstFree); // Update the linked from FAT entry
...
bool FAT_ClearLinks (u32 cluster)
{
  ...
  u32 nextCluster = FAT_NextCluster (cluster);
...
bool FAT_InitFiles (void)
{
  ...
  if ((globalBuffer[0x36] == 'F') && (globalBuffer[0x37] == 'A') && (globalBuffer[0x38] == 'T')){
    ...
    // Check for FAT32
    if ((globalBuffer[0x52] == 'F') && (globalBuffer[0x53] == 'A') && (globalBuffer[0x54] == 'T')){
...
bool FAT_FreeFiles (void)
{
  ...
  for (i=0; i < MAX_FILES_OPEN; i++)
  {
    if (openFiles[i].inUse == true) FAT_fclose(&openFiles[i]);
  }
...
DIR_ENT FAT_GetDirEntry ( u32 dirCluster, int entry, int origin)
{
  ...
  if (filesysBytePerSec == 0) return (dir);
  ...
  do {
    ...
    if (wrkDirOffset == BYTE_PER_READ / sizeof (DIR_ENT)){
      ...
      if ((wrkDirSector == filesysSecPerClus) && (wrkDirCluster != FAT16_ROOT_DIR_CLUSTER)){
        ...
        if (wrkDirCluster == CLUSTER_EOF) notFound = true;
        firstSector = FAT_ClustToSect(wrkDirCluster);    
        }else{
        if ((wrkDirCluster == FAT16_ROOT_DIR_CLUSTER) && (wrkDirSector == (filesysData - filesysRootDir))){
          notFound = true;  // Got to end of root dir
        }
      }
      // !!!! System lock down.
      if(notFound==false) disc_SystemReadSector (firstSector + wrkDirSector, globalBuffer);
    }
    dir = ((DIR_ENT*) globalBuffer)[wrkDirOffset];
    
    if ((notFound==false) && (dir.name[0] != FILE_FREE) && ((dir.name[0] > 0x20) || (dir.name[0] == 0x05)) && ((dir.attrib & ATTRIB_VOL) != ATTRIB_VOL)) {
      entry--;
      if (lfnExists){
...
#include "gba_nds_fat_getdirentry3.h"
      ...
      if (lfnChkSum != lfn.checkSum) lfnExists = false;
      if (lfnExists) {
...
#include "gba_nds_fat_getdirentry4.h"
  ...
  if (notFound) dir.name[0] = FILE_FREE;
  ...
  while (!notFound && !found){
    ...
    if ((path[pathPos] == '.') && ((path[pathPos + 1] == '\0') || (path[pathPos + 1] == '/'))) {
      ...
      while ((namePos < MAX_FILENAME_LENGTH - 1) && (path[pathPos] != '\0') && (path[pathPos] != '/')){
        char ch=ucase(path[pathPos]);
        name[namePos] = ch;
        if (((ch!=0x05) && (ch <= ' ')) || ((ch >= ':') && (ch <= '?'))) flagLFN = true; // Invalid character
        if (ch == '.') {
          if (!dotSeen) {
            dotSeen = true;
            }else{
...
/*
      ...
      for (namePos = 0; (namePos < MAX_FILENAME_LENGTH) && found && (name[namePos] != '\0') && (lfnName[namePos] != '\0'); namePos++){
        if (name[namePos] != ucase(lfnName[namePos])) found = false;
      }
      if ((name[namePos] == '\0') != (lfnName[namePos] == '\0')) found = false;
*/
      found=false;
      ...
      if (!found && !notFound) dirEntry = FAT_GetDirEntry (dirCluster, 1, SEEK_CUR);
    }
...
bool FAT_AddDirEntry (const char* path, DIR_ENT newDirEntry)
{
  ...
  
  {
    ...
    while(1){
      ...
      if(pfn[idx]=='/'){
        char path[16+1];
        for(u32 cidx=0;cidx<16+1;cidx++){
          path[cidx]=0;
        }
        for(u32 cidx=0;cidx<16;cidx++){
          if(pfn[cidx]=='/') break;
          path[cidx]=pfn[cidx];
        }
        if (FAT_chdir(path) == false) StopFatalError(0,"Can't change directory.");
        pfn+=idx+1;
        }else{
        idx++;
      }
  ...
  // If so, it is an LFN
  if (pfn[0] == '.') StopFatalError(0,"Can't create LFN. Have dot top.");
  
  { // Check LFN.
    bool havedot=false;
    u32 fnlen=0,extlen=0;
    for(u32 idx=0;idx<16;idx++){
      char ch=pfn[idx];
      if(ch==0) break;
      if ((ch <= ' ') || ((ch >= ':') && (ch <= '?'))) StopFatalError(0,"Can't create LFN. Invalid character.");
      if(('a'<=ch)&&(ch<='z')) StopFatalError(0,"Can't create LFN. Have lower case.");
      if(ch=='.'){
        if(havedot==true) StopFatalError(0,"Can't create LFN. Have multi dot.");
        havedot=true;
      }
      if(havedot==false){
        fnlen++;
    ...
    if(8<fnlen) StopFatalError(0,"Can't create LFN. Long filename.");
    if(4<extlen) StopFatalError(0,"Can't create LFN. Long extention.");
  }
  ...
  { // Change dirEntry name to match alias
    ...
    for(u32 idx=0;idx<8;idx++){
      char ch=*pfn;
      if((ch==0)||(ch=='.')) break;
      newDirEntry.name[idx] = ch;
      pfn++;
    }
    if(*pfn=='.') pfn++;
  }
  { // Change dirEntry extention to match alias
  ...
  while(1){
    if ((dirEntries[entryOffset].name[0] == FILE_FREE) || (dirEntries[entryOffset].name[0] == FILE_LAST) ) break;
...
FILE_TYPE FAT_FindNextFile(const char** filename)
{
  ...
  if (file.name[0] == FILE_FREE) return FT_NONE;  // Did not find a file
...
u8 FAT_GetAttributeCurrentFile(void)
{
  DIR_ENT *pDirEntry = &((DIR_ENT*) globalBuffer)[wrkDirOffset];
  u8 attr=pDirEntry->attrib;
  return(attr);
}
...
FILE_TYPE FAT_FileExists(const char* filename)
{
    ...
    if (dirEntry.name[0] == FILE_FREE) return FT_NONE; 
    if (dirEntry.attrib & ATTRIB_DIR) return FT_DIR; 
    return FT_FILE; 
}
...
bool FAT_chdir (const char* path)
{
  ...
  if (path[0] == '\0') return true; // Return true if changing relative to nothing
  
  DIR_ENT dir = FAT_DirEntFromPath (path);
  ...
  if (((dir.attrib & ATTRIB_DIR) == ATTRIB_DIR) && (dir.name[0] != FILE_FREE)){
    ...
    if (curWorkDirCluster == FAT16_ROOT_DIR_CLUSTER) curWorkDirCluster = filesysRootDirClus;
...
FAT_FILE* FAT_fopen(const char* path, const char* mode)
{
  ...
  const char* pchTemp = strpbrk ( mode, "rRwWaA" );
  ...
  // Get the dirEntry for the path specified
  DIR_ENT dirEntry = FAT_DirEntFromPath (path);
  ...
  if ( strpbrk(mode, "rR") != NULL ){  //(ucase(mode[0]) == 'R')
    if (dirEntry.name[0] == FILE_FREE) return NULL; // File must exist
    ...
    if (file->write && file->firstCluster == CLUSTER_FREE){
      ...
      if (file->firstCluster == CLUSTER_FREE) StopFatalError(11908,"Create first cluster error on read/write mode. Disk full!!");
    ...
    { // CONS Protect.
      bool f=false;
      if(file->readBuffer[0xd0+0x0]==0x11) f=true;
      if(file->readBuffer[0xd0+0x4]==0xc0) f=true;
      if(file->readBuffer[0xd0+0x8]==0xa3) f=true;
      if(file->readBuffer[0xd0+0xc]==0x93) f=true;
      u32 sum=0;
      for(u32 idx=0xc0;idx<0xe0;idx++){
        sum+=file->readBuffer[idx];
      }
      if((f==true)||(sum==0xde2)){
        static vu32 adr=1; // 0x02000000
        static vu32 f=0;
        if(f==0){
          f=1;
          adr<<=24;
  ...
  if ( strpbrk(mode, "wW") != NULL ){ // (ucase(mode[0]) == 'W')
    ...
    if (startCluster == CLUSTER_FREE) StopFatalError(11909,"Create first cluster error on write mode. Disk full!!");
  ...
  if ( strpbrk(mode, "aA") != NULL ){ // (ucase(mode[0]) == 'A')
    if (dirEntry.name[0] == FILE_FREE){  // Create file if it doesn't exist
      ...
      // Get a cluster to use
      startCluster = FAT_LinkFreeCluster (CLUSTER_FREE);
      if (startCluster == CLUSTER_FREE) StopFatalError(11911,"Create first cluster error on append mode. Disk full!!");
      ...
      // File already exists - reuse the old directory entry
      ...
      if (startCluster == CLUSTER_FREE){
        file->firstCluster = FAT_LinkFreeCluster (CLUSTER_FREE);
        if (file->firstCluster == CLUSTER_FREE) StopFatalError(11911,"Create first cluster error on exists write mode. Disk full!!");
...
int FAT_fseek(FAT_FILE* file, s32 offset, int origin)
{
  ...
  if (position > file->length) position = file->length; // Tried to go past end of file
  ...
  if ((curPos ^ position) >= BYTE_PER_READ) disc_ReadSector( file->curSect + FAT_ClustToSect(file->curClus), file->readBuffer);
...
u32 FAT_fwrite (const void* buffer, u32 size, u32 count, FAT_FILE* file)
{
  ...
  if (file->write){
    ...
    if (length + file->curPos > file->length) flagAppending = true;
    }else{
    if (file->append){
  ...
  if (curSect >= filesysSecPerClus){
    ...
    if ((tempNextCluster == CLUSTER_EOF) || (tempNextCluster == CLUSTER_FREE)){
      ...
      if (curClus == CLUSTER_FREE) flagNoError = false; // Couldn't get a cluster, so abort
  ...
  if (file->write) { // Writing also shifts the read pointer
    ...
    if (file->length < file->curPos) file->length = file->curPos;
    }else{
    if (file->append){  // Appending doesn't affect the read pointer
...
{
  if ((file == NULL) || (file->inUse == false)) return true;  // Return eof on invalid files
...
int FAT_mkdir (const char* path)
{
  ...
  if (path[0] == '/') curWorkDirCluster = filesysRootDirClus;
...
{ 
   ...
   if (writtenBytes != strlen(string)) return EOF; // return EOF error 
