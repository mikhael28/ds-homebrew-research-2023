...
{
  _consolePrintf("FAT2: Set Prohibition16bitsReadAccess flag.\n");
...
{
  if(((u32)buffer&1)!=0){
    _consolePrintf("FAT_fread_fast: Align error. 0x%08x, %d,%d, 0x%08x.\n",buffer,size,count,file);
    while(1);
  }
  
  if((is64kCluster==true)||(Prohibition16bitsReadAccess==true)) return(FAT_fread(buffer,size,count,file));
  ...
  if (curSect >= filesysSecPerClus)
  {
    ...
    if ((remain == 0) && (tempNextCluster == CLUSTER_EOF)){
      ...
      if (curClus == CLUSTER_FREE) flagNoError = false;
    }
  }
